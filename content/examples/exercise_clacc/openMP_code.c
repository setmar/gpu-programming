...
int main (int argc, char** argv) {
  ...
  // many iterations
  #pragma omp target data map(tofrom: array) map(alloc: arr_new)
  while (error > MAX_ERROR && iterations < MAX_ITER) {
    error = 0.;
    #pragma omp target teams map(alloc: arr_new,array) map(tofrom: error) firstprivate(NUM_ELEMENTS) reduction(max: error)
    ...
    #pragma omp target teams map(alloc: array,arr_new) firstprivate(NUM_ELEMENTS)
    #pragma omp distribute collapse(2)
    for (int i = 1; i < NUM_ELEMENTS - 1; i++) {
      for (int j = 1; j < NUM_ELEMENTS - 1; j++) {
